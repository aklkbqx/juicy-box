...
function showMessageAlert(string $type_msg)
{
    if (isset($_SESSION[$type_msg])) { ?>
        <div class='position-fixed opacity-0' style="top:0rem;right:2rem;z-index:99999" id="showMsg">
...
{
    global $row;
    $orderProducts = ($_SERVER["REQUEST_URI"] === "/" || $_SERVER["REQUEST_URI"] === "index.php") ? "#orderProducts" : "/#orderProducts";
    ?>
    <div class="nav-group-link">
        <div class="d-flex align-items-center gap-0">
            <a href="/" class="navLink">
                <i class="fa-solid fa-house d-block"></i>
                <div class="d-none d-xl-block">หน้าแรก</div>
            </a>
            <a href="<?= $orderProducts; ?>" class="navLink">
                <i class="fa-solid fa-mug-saucer d-block"></i>
                <div class="d-none d-xl-block">เมนูชาและกาแฟ</div>
            </a>
            <a href="<?= linkPage("orders.php") ?>" class="navLink">
                <i class="fa-solid fa-list-ul d-block"></i>
                <div class="d-none d-xl-block">คำสั่งซื้อ</div>
                <?php if (isset($_SESSION["user_login"])) { ?>
                    <div class="bg-danger rounded-circle position-absolute d-flex justify-content-center align-items-center" style="width: 30px;height:30px;top:-10px;right:-10px;font-size: 15px;">
                        <?= sql("SELECT COUNT(*) AS `count` FROM `orders` WHERE `user_id` = ?", [$row["user_id"]])->fetch(PDO::FETCH_ASSOC)["count"]; ?>
                    </div>
                <?php } ?>
            </a>
            <a href="<?= linkPage("cart.php") ?>" class="navLink">
                <i class="fa-solid fa-cart-shopping d-block"></i>
                <div class="d-none d-xl-block">ตะกร้าสินค้า</div>
                <?php if (isset($_SESSION["user_login"])) { ?>
                    <div class="bg-danger rounded-circle position-absolute d-flex justify-content-center align-items-center" style="width: 30px;height:30px;top:-10px;right:-10px;font-size: 15px;">
                        <?= sql("SELECT COUNT(*) AS `count` FROM `carts` WHERE `user_id` = ?", [$row["user_id"]])->fetch(PDO::FETCH_ASSOC)["count"]; ?>
                    </div>
                <?php } ?>
            </a>
        </div>
