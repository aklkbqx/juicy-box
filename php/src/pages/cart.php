...
$user_id = isset($_SESSION["user_login"]) ? $_SESSION["user_login"] : (isset($_SESSION["admin_login"]) ? msg("ไม่สามารถไปยังหน้า Cart ได้", "warning", "../admin/") : msg("กรุณาเข้าสู่ระบบก่อน!", "warning", "./login.php"));
if ($user_id) {
...
<body>
    ...
    <div class="p-2">
        <div class="container bg-white rounded-5 shadow p-3" style="margin-top: 7rem;">
            <?php $carts = sql("SELECT `cart_id`,`product_id`,`amount`,`drink_type` FROM `carts` WHERE `user_id` = ?", [$row["user_id"]]);
            if ($carts->rowCount() > 0) {
            ?>
                <div class="row">
                    <div class="col-xxl-7">
                                            ...
                                            <div class="col-sm-5 col-12 d-flex align-items-center">
                                                <div class="d-flex align-items-center gap-4">
                                                    ...
                                                    <div class="d-flex flex-column gap-2">
                                                        ...
                                                        <h6><span class="fw-bold">ประเภทการชง:</span> <?= $cart["drink_type"] ?></h6>
                                                    </div>
                                                </div>
                                            </div>
                                                ...
                                                <div class="w-100">
                                                    ...
                                                    <div class="row">
                                                        <div class="col-sm-2 col-4 p-0 d-flex align-items-center justify-content-center">
                                                            <h6>
                                                                <span data-eachitem="<?= $cart["cart_id"]; ?>"><?= $product["price"]; ?></span> บาท
                                                            </h6>
                                                        </div>
                                                        <div class="col-sm-5 col-4 p-0 d-flex align-items-center justify-content-center">
                                                            <div class="d-flex overflow-hidden border rounded-4" style="width: 110px;">
                                                                <button type="button" class="btn btn-light border-0 rounded-0 border-0" onclick="minusAmountItem(<?= $cart['cart_id']; ?>)">-</button>
                                                                <input oninput="changeAmountItem(<?= $cart['cart_id']; ?>)" data-amountitem="<?= $cart['cart_id']; ?>" type="number" class="form-control border-0 text-center" style="width: 50px;box-shadow: none !important;outline: none;" value="<?= $cart["amount"] ?>">
                                                                <button type="button" class="btn btn-light border-0 rounded-0 border-0" onclick="plusAmountItem(<?= $cart['cart_id']; ?>)">+</button>
                                                            </div>
                                                        </div>
                                                        <div class="col-sm-2 col-4 p-0 d-flex align-items-center justify-content-center">
                                                            <div style="font-size: 14px;">
                                                                <span data-total-eachitem="<?= $cart['cart_id']; ?>"><?= formatNumberWithComma($product["price"] * $cart["amount"]) ?></span> บาท
                                                            </div>
                                                        </div>
                                                        <div class="col-sm-3 col-12 p-0 d-flex justify-content-sm-center justify-content-end">
                                                            <button onclick="deleteItem(<?= $cart['cart_id']; ?>)" type="button" class="btn btn-danger rounded-4 me-2 me-sm-0">
    ...
    <script>
        ...
        const updateCartItem = (cart_id, amount) => {
            $.ajax({
        ...
        const plusAmountItem = (cart_id) => {
            const eachItem = $(`[data-eachitem=${cart_id}]`);
            const amountItem = $(`[data-amountitem=${cart_id}]`);
            const totalItem = $(`[data-total-eachitem=${cart_id}]`);
            const drink_type = $(`[data-drink_type=${cart_id}]`).val(); // เพิ่มการดึงขนาด
            ...
            updateCartItem(cart_id, newAmount, drink_type);
        };

        const minusAmountItem = (cart_id) => {
            const eachItem = $(`[data-eachitem=${cart_id}]`);
            const amountItem = $(`[data-amountitem=${cart_id}]`);
            const totalItem = $(`[data-total-eachitem=${cart_id}]`);
            const drink_type = $(`[data-drink_type=${cart_id}]`).val(); // เพิ่มการดึงขนาด
        ...
        const changeAmountItem = (cart_id) => {
            const eachItem = $(`[data-eachitem=${cart_id}]`);
            const amountItem = $(`[data-amountitem=${cart_id}]`);
            const totalItem = $(`[data-total-eachitem=${cart_id}]`);
            const drink_type = $(`[data-drink_type=${cart_id}]`).val(); // เพิ่มการดึงขนาด
